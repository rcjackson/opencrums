...
def classifier_model(shape, the_dict, dataset):
    ...
    inp_layer = Input(shape=(height, width, 5), name="input_all")
    mpool_1 = inp_layer
...
def run(config: dict):
    ...
    keys = list(x_ds_train.keys())
    x_ds_train['input_all'] = np.stack([x_ds_train[k] for k in keys], axis=-1)
    x_ds_test['input_all'] = np.stack([x_ds_test[k] for k in keys], axis=-1)
    x_ds_valid['input_all'] = np.stack([x_ds_valid[k] for k in keys], axis=-1)
    ...
    history = model.fit(
            ...
            callbacks=[EarlyStopping(patience=50, monitor="val_acc"), ReduceLROnPlateau(min_lr=1e-6,
